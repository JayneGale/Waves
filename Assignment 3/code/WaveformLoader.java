...
public class WaveformLoader {
    ...
    public static ArrayList<Double> doLoad() {
        ...
        if (!filename.endsWith(".wav") && !filename.endsWith(".WAV")) {
            JOptionPane.showMessageDialog(null, "Not a  wav file");
            return null;
        }
        ...
        try {
            ...
            for (int i = 0; i < numSamples; i++) {
                ...
                double value = 0;
                if (fsize == 1) {
                    value = ((double) frame[0]) * 0xFFFF / 0xFF;
                }    // scale 8 bit to 16 bit
                else {
                    value = (double) ((frame[0] & 0xFF) | frame[1] << 8);
                } // read 16 bit
            ...
            JOptionPane.showMessageDialog(null, "Not a valid audio file: " + e);
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "Loading the wav file failed: " + e);
        }
    ...
    public static void doSave(ArrayList<Double> waveform, double scale) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Writing the wav file failed: " + e);
        }
    }

    public static void doSaveS(ArrayList<ComplexNumber> spectrum)  {
        try {
//            AudioSystem.write(audioStream, AudioFileFormat.Type.WAVE, new File(filename));
            FileWriter writer = new FileWriter("spectrum.txt");
        for (ComplexNumber c : spectrum)
            writer.write(c.getRe() + " " + c.getIm() + System. lineSeparator());
        writer.close();
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "Writing the spectrum file failed: " + e);
        }
    }
}
